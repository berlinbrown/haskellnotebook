...
-export([delete_factoid/3, change_factoid/3]).

start() ->
    spawn(factoid_plugin, init, []).

init() ->
    msg_dispatch:add(["PRIVMSG"]),
    loop().
...
loop() ->
    receive
        {"PRIVMSG", [From, [$# | _Channel] = To, Message], [Bot]} ->
        ...
        {"PRIVMSG", [_From, _To, _Message], [_Bot]} ->
            ok
    end,
    factoid_plugin:loop().
...
delete_factoid(To, Bot, Factoid) ->
    {ok, Name} = bot_manager:fetch_name(Bot),
    case factoid:delete(httpd_util:to_lower(string:strip(Factoid)), {To, Name}) of
        ok ->
            Factoid ++ " deleted";
        {error, Reason} ->
            {error, Reason}
...
bot_trigger(Nick, Message) ->
    ...
    NormNick = httpd_util:to_lower(Nick),
    NormMsg = httpd_util:to_lower(Message),
...
should_handle(From, _Channel, Message, Bot) ->
    ...
    case lists:member(httpd_util:to_lower(Nick), ["candide", "geordi", "nolyc"]) of
        true ->
            false;
        false ->
